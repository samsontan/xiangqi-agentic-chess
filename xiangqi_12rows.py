#!/usr/bin/env python3
"""
Xiangqi with 12 Rows - File labels embedded in rows 1 and 12 for perfect alignment
"""
...
def draw_12row_board(game):
    """Draw board with 12 rows, embedding file labels in rows 1 and 12"""
    piece_map = {
    ...
    # Row 12 - File labels at top
    print("12 a b c d e f g h i 12")

    # Rows 11-2 (the actual game board, rows 10-1 in game coordinates)
    for row in range(10):
        rank = 10 - row
        line = f"{rank+1:2} "
        ...
        for col in range(9):
            ...
            line += " "  # Space after each position

        line += f"{rank+1:2}"
        print(line)

        # River marker
        if row == 4:  # After rank 6 (which is row 4 in 0-based indexing)
            print("   ─ ─ ─ 楚河 汉界 ─ ─ ─")

    # Row 1 - File labels at bottom
    print(" 1 a b c d e f g h i  1")
    print()

class TwelveRowXiangqiGame(FastXiangqiGame):
    def display_board(self):
        """Use 12-row board display"""
        draw_12row_board(self)

def main():
    """Main game with 12-row board"""
    print("🎮 XIANGQI WITH 12-ROW BOARD")
    print("=" * 50)
    print("🔴 You are RED, ⚫ AI is BLACK")
    print("🚀 AI responds quickly (under 2 seconds)")
    print("📍 File labels (a-i) are in rows 1 and 12 for perfect alignment!")
    print("Enter moves like: e4-e5 (with dash)")
    print("Type 'quit' to exit")
    print("=" * 50)

    game = TwelveRowXiangqiGame()
    move_count = 0
