#!/usr/bin/env python3
"""
Xiangqi with Circled Letters for File Labels
"""
...
def draw_circled_board(game):
    """Draw board with circled letters for file labels"""
    piece_map = {
    ...
    # Circled letters for file labels
    circled_letters = ['ⓐ', 'ⓑ', 'ⓒ', 'ⓓ', 'ⓔ', 'ⓕ', 'ⓖ', 'ⓗ', 'ⓘ']

    # Header with circled letters
    print("\n  " + " ".join(circled_letters))
    ...
    # Footer with circled letters
    print("  " + " ".join(circled_letters) + "\n")

class CircledXiangqiGame(FastXiangqiGame):
    def display_board(self):
        """Use circled letter board display"""
        draw_circled_board(self)

def main():
    """Main game with circled letters"""
    print("🎮 XIANGQI WITH CIRCLED FILE LETTERS")
    print("=" * 50)
    print("🔴 You are RED, ⚫ AI is BLACK")
    print("🚀 AI responds quickly (under 2 seconds)")
    print("📍 File labels are circled (ⓐ-ⓘ) for easy identification!")
    print("Enter moves like: e4-e5 (with dash)")
    print("Type 'quit' to exit")
    print("=" * 50)

    game = CircledXiangqiGame()
    move_count = 0
