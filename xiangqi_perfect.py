#!/usr/bin/env python3
"""
Perfectly Aligned Xiangqi - Uses exact character positioning
"""
...
def draw_perfect_board(game):
    """Draw board with exact character positioning for perfect alignment"""
    piece_map = {
    ...
    for row in range(10):
        rank = 10 - row
        # Build line with exact positioning
        pieces = []
        ...
        for col in range(9):
            ...
            if piece:
                ...
                if piece[0] == 'red':
                    pieces.append(f"\033[91m{symbol}\033[0m")
                else:
                    pieces.append(f"\033[94m{symbol}\033[0m")
            else:
                pieces.append("·")

        # Format line with exact spacing to match header
        line = f"{rank:2} {' '.join(pieces)} {rank:2}"
        print(line)
...
class PerfectXiangqiGame(FastXiangqiGame):
    def display_board(self):
        """Use perfect alignment display"""
        draw_perfect_board(self)

def main():
    """Main game with perfect alignment"""
    print("🎮 PERFECTLY ALIGNED XIANGQI")
    print("=" * 50)
